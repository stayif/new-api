...
const (
	AttestedRelaySchema = "newapi.attested_relay.v2"
...
func PreflightAttestedRelay(c *gin.Context, info *RelayInfo) (*AttestedRelayState, error) {
	...
	expectedAuthorization := "v2=" + signAttestation(secret, []byte(authPayload))
...
func (s *AttestedRelayState) SetCompiledRequest(compiledBody []byte) error {
	if s == nil {
		return errors.New("attested relay state is missing")
	}
...
func (s *AttestedRelayState) ObserveClaudeResponse(response *dto.ClaudeResponse) error {
	...
	case "ping":
		if response.Id != "" || response.Role != "" || len(response.Content) != 0 ||
			...
			return errors.New("provider ping contains unexpected semantic fields")
		}
	case "message_start":
	...
	case "content_block_start":
		...
		case "thinking":
			if response.ContentBlock.Thinking != nil {
				thinking := *response.ContentBlock.Thinking
				if strings.TrimSpace(thinking) != "" {
					if s.TextChars > 0 {
						return errors.New("provider emitted reasoning after ordinary text")
					}
					s.ReasoningChars += len([]rune(thinking))
				}
			}
		case "text":
			if response.ContentBlock.Text != nil {
				text := *response.ContentBlock.Text
...
func (s *AttestedRelayState) BuildSuccessEnvelope(info *RelayInfo, usage *dto.Usage, newAPIRequestID, executionRequestID string) (*AttestedRelayEnvelope, error) {
	...
	if s.CompiledBodySHA256 == "" {
		return nil, errors.New("compiled provider request receipt is missing")
	}
	if s.Billing == nil {
	...
	return &AttestedRelayEnvelope{
		...
		Receipt:   receipt,
		Signature: "v2=" + signAttestation(secret, payload),
	}, nil
}
