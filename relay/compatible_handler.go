...
func TextHelper(c *gin.Context, info *relaycommon.RelayInfo) (newAPIError *types.NewAPIError) {
	...
	if passThroughGlobal || info.ChannelSetting.PassThroughBodyEnabled {
		...
		if relaycommon.IsAttestationRequested(c) {
			...
			if !ok {
				return types.NewErrorWithStatusCode(fmt.Errorf("selected provider has no native token count receipt"), types.ErrorCodeCountTokenFailed, http.StatusBadGateway, types.ErrOptionWithSkipRetry())
			}
			finalJSON, countReceipt, err := compiler.CompileAndCountProviderTokens(c, info, jsonData)
			if err != nil {
				return types.NewErrorWithStatusCode(err, types.ErrorCodeCountTokenFailed, http.StatusBadGateway, types.ErrOptionWithSkipRetry())
			}
			jsonData = finalJSON
			if err := state.SetCompiledRequest(jsonData, countReceipt); err != nil {
				return types.NewErrorWithStatusCode(err, types.ErrorCodeCountTokenFailed, http.StatusBadGateway, types.ErrOptionWithSkipRetry())
			}
		}
